...
TfLiteStatus GetAudioSamples(tflite::ErrorReporter* error_reporter,
  ...
  for (int i = 0; i < duration_sample_count; ++i) {
    ...
    //g_audio_capture_buffer[capture_index] = 0; // actually it's bc mic hears silence (0s) that the later predictions are inaccurate (mainly mixture of no and unknown)
  }
  
  *audio_samples_size = kMaxAudioSampleSize;
  *audio_samples = g_audio_output_buffer;

  // maybe reset array to 0s? or g_audio_capture_buffer to 0s?
/*   for (int i = 0; i < duration_sample_count; ++i) {
